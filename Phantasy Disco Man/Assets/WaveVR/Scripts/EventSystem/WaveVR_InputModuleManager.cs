...
public class WaveVR_InputModuleManager : MonoBehaviour
{
    ...
    {
        WVR_GazeTriggerType _gazetype = this.gazeTriggerType_System;
        if (!Application.isEditor)
        {
            _gazetype = Interop.WVR_GetGazeTriggerType ();
        }
        updateGazeTriggerType_System (_gazetype);
        ...
        if (this.OverrideSystemSettings)
        {
            ...
            updateGazeTriggerType_User ();
            if (gazeTriggerType_User != _gazetype)
            {
    ...
    private void updateGazeTriggerType_System(WVR_GazeTriggerType type)
    {
        if (gazeTriggerType_System != type)
        {
            if (gazeTriggerType_User != type)
            {
                gazeTriggerType_System = type;
            }
        }
    }
